...
ScriptEngine::~ScriptEngine() { delete state_; }

void ScriptEngine::exportFunction(const String & sym, const String & nameSpace,
                                  const String & docstring, int argc,
                                  void * (*proc)()) {
    auto & env = state_->context_.topLevel();
    lisp::Persistent<lisp::String> doc(
        env, env.create<lisp::String>(docstring.c_str(), docstring.length()));
    const auto param = reinterpret_cast<lisp::ObjectPtr (*)(
        lisp::Environment &, const lisp::Arguments &)>(proc);
    auto fn = env.create<lisp::Function>(doc.get(), size_t(0), param);
    env.setGlobal(sym, nameSpace, fn);
}

void ScriptEngine::setGlobal(const String & sym, const String & nameSpace,
                             unsigned value) {
    auto & env = state_->context_.topLevel();
